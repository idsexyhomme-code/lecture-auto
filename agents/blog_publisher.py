...
class BlogPublisher(BaseAgent):
    ...
    system_prompt = (
        ...
        "  - 본론 3: 왜 첫 시도부터 결과가 나오는지 1-2가지\n"
        "  - 마무리: 첫 차시 1개부터 보라는 한 줄 + 사이트 링크\n"
        "  ⚠️ 소제목은 '이게 작동하는 이유'·'왜 이게 어려운지' 같은 뻔한 템플릿 라벨 금지. "
        "내용을 한 줄로 말하는 구체적 제목으로 짓는다 (예: '선택지가 없으면 손이 먼저 움직인다').\n"
    )
...
⚠️ 소제목(<h2>) 규칙 — *가장 중요*:
  '이게 작동하는 이유', '왜 이게 어려운지', 'Claude로 어떻게 하는지', '지금 시작하기' 같은
  뻔한 템플릿 라벨을 *그대로 쓰지 마라*. 매번 글 내용을 한 줄로 말하는 구체적 제목으로 새로 지어라.
  나쁜 예: "이게 작동하는 이유"  →  좋은 예: "선택지가 없으면 손이 먼저 움직인다"
  나쁜 예: "왜 이게 어려운지"  →  좋은 예: "집중이 안 되는 건 의지력 문제가 아니다"

구조 (HTML, 1500-2500자) — 아래는 *흐름 설명*일 뿐, 제목 문구를 베끼지 마라:
  <h2>(도입 — 구체적 장면 한 줄)</h2>
  <p>회원님 일상에서 실제 일어나는 장면 묘사. 시간·숫자·구체 사례.</p>

  <h2>(왜 이게 어려운지를 — 내용으로 말하는 구체적 제목)</h2>
  <p>2-3 문단. 보통 사람의 진짜 이유를 솔직하게.</p>

  <h2>(Claude를 어떻게 쓰는지를 — 내용으로 말하는 구체적 제목)</h2>
